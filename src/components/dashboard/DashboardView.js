...
const DashboardView = ({ user, onNewOrder }) => {
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 h-full">
            {availableNavItems.map(item => (
                <button
                    key={item.id}
                    onClick={() => handleNavigate(item.id, item.action)}
                    className={`flex flex-col items-center justify-center text-center p-4 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white ${item.color} text-gray-800 font-semibold transition-transform transform hover:scale-105`}
                >

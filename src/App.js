        ...
        <div className="p-4 md:p-8">
            ...
            <div className="relative max-w-2xl mx-auto">
                <div className="flex items-center bg-white dark:bg-gray-700 rounded-full shadow-lg">
                    ...
                    <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Wpisz kod kreskowy, kod produktu lub nazwę..." className="w-full p-4 bg-transparent focus:outline-none text-gray-900 dark:text-white" />
                </div>
...
const PickingView = () => {
    ...
    useEffect(() => {
        ...
        const availableSuggestions = toPickItems.filter(item => 
            ...
            (item.barcode && item.barcode.includes(inputValue))
        );
    ...
    if (!selectedOrder) {
        return (<div className="p-4 md:p-8"><h1 className="text-3xl font-bold mb-6 text-gray-800 dark:text-white">Kompletacja Zamówień</h1><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{orders.map(order => (<div key={order._id} onClick={() => handleSelectOrder(order)} className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-md cursor-pointer hover:shadow-lg hover:scale-105 transition-all"><p className="font-bold text-lg text-indigo-600 dark:text-indigo-400">{order.customerName}</p><p className="text-sm text-gray-500 dark:text-gray-400">Autor: {order.author}</p><p className="text-sm text-gray-500 dark:text-gray-400">{new Date(order.date).toLocaleDateString()}</p></div>))}</div></div>);
    }
    ...
    return (
        <div className="p-4 md:p-8">
            ...
            <div className="relative max-w-xl mb-8">
                <input type="text" ref={searchInputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown} placeholder="Skanuj lub wyszukaj produkt do skompletowania..." className="w-full p-4 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
                {suggestions.length > 0 && <ul className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg shadow-xl max-h-60 overflow-y-auto">{suggestions.map(p => <li key={p._id} onClick={() => handlePickItem(p)} className="p-3 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600"><p className="font-semibold text-gray-800 dark:text-gray-100">{p.name}</p></li>)}</ul>}
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div>
                    <h2 className="text-2xl font-semibold mb-4 text-gray-800 dark:text-white">Do skompletowania ({toPickItems.length})</h2>
...
const HomeView = ({ user, setActiveView }) => {
    ...
    const StatCard = ({ title, value, icon, color, onClick }) => (
        <div onClick={onClick} className={`bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md flex items-center text-left transition-all hover:shadow-xl hover:scale-105 ${onClick ? 'cursor-pointer' : ''}`}>
            ...
            <div className="ml-4">
                <p className="text-3xl font-bold text-gray-800 dark:text-white">{isLoading ? '...' : value}</p>
    ...
    return (
        <div className="p-4 md:p-8">
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <StatCard title="Wszystkie zamówienia" value={stats?.totalOrders} icon={<Archive className="h-8 w-8 text-blue-600 dark:text-blue-300" />} color="bg-blue-100 dark:bg-blue-900" onClick={() => setActiveView('orders')} />
                <StatCard title="Całkowity przychód" value={`${(stats?.totalRevenue || 0).toFixed(2)} PLN`} icon={<BarChart2 className="h-8 w-8 text-indigo-600 dark:text-indigo-300" />} color="bg-indigo-100 dark:bg-indigo-900" />
            </div>
        </div>
    );
};

...
import { Search, List, Wrench, User, Sun, Moon, LogOut, FileDown, Printer, Save, CheckCircle, AlertTriangle, Upload, Trash2, XCircle, UserPlus, KeyRound, PlusCircle, MessageSquare, Archive, Edit, Home, Menu, Filter, RotateCcw, FileUp, GitMerge, Eye, Target, Trophy, Crown, Map, ChevronsUpDown, ChevronUp, ChevronDown } from 'lucide-react';
import { DragDropContext, Droppable, Draggable } from 'react-beautiful-dnd';
import { format, parseISO, eachDayOfInterval, isValid } from 'date-fns';
import { pl } from 'date-fns/locale';
import { GoogleMap, useLoadScript, Marker, DirectionsRenderer } from '@react-google-maps/api';
import './NewStyles.css'; // Import nowego pliku CSS
...
function App() {
    ...
    const navItems = [
        ...
        { id: 'delegations', label: 'Delegacje', icon: Map, roles: ['user', 'administrator'] },
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <div><p className="font-semibold">{user.username}</p><p className="text-sm text-gray-500">{user.role}</p></div>
                             <div className="flex items-center">
                                ...
                                <Tooltip text="Wyloguj"><button onClick={handleLogout} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600"><LogOut className="w-5 h-5 text-gray-500" /></button></Tooltip>
                             </div>
                        </div>
